...
function SignUpPlayerScreen({ navigation, addUser }) {
    ...
    var handleSubmitSignup = async () => {
        ...
        if (response.result === true && check === true) {
            addUser(response.userInfos)
            navigation.navigate('AccessTeamForPlayerScreen')
        } else {
    ...
    var tabErrorsSignup = listErrorsSignup.map((error, i) => {
        return (<Text key={i} style={{ color: 'red', marginRight: 'auto', marginBottom: 5 }}>{error}</Text>)
    })
        ...
        <ScrollView>
            ...
            <View style={{ alignItems: 'center', width: '95%', alignSelf: 'center' }}>
              ...
              <DatePicker
                ...
                <Button onPress={() => handleSubmitSignup()} title="REJOINDRE UNE TEAM" buttonStyle={{ backgroundColor: "#FE7235", width: 325, height: 50, borderRadius: 10, marginBottom: 20 }} />
